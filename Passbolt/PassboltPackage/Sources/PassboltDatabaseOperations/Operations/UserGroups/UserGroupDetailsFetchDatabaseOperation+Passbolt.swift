...
extension UserGroupDetailsFetchDatabaseOperation {
  ...
  @MainActor fileprivate static func load(
    ...
    nonisolated func executeAsync(
      _ input: UserGroup.ID
    ) async throws -> UserGroupDetailsDSV {
      try await execute(
        input,
...
extension FeatureFactory {
  ...
  internal func usePassboltUserGroupDetailsFetchDatabaseOperation() {
    self.use(
      .disposable(
        UserGroupDetailsFetchDatabaseOperation.self,
        load: UserGroupDetailsFetchDatabaseOperation
          .load(features:)
      )
    )
  }
