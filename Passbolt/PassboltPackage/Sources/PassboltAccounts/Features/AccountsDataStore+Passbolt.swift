...
extension AccountsDataStore {
  ...
  @MainActor fileprivate static func load(
    ...
    @Sendable func ensureDataIntegrity() throws {
      Diagnostics.logger.info("Verifying data integrity...")
      defer {
        Diagnostics.logger.info("...data integrity verification finished")
      }
      ...
      Diagnostics.debug("Stored accounts list: \(storedAccountsList)")
      ...
      Diagnostics.debug("Stored accounts: \(storedAccounts)")
      ...
      Diagnostics.debug("Stored account profiles: \(storedAccountsProfiles)")
      ...
      Diagnostics.debug("Stored account mfa tokens: \(storedAccountMFATokens)")
      ...
      Diagnostics.debug("Stored server fingerprints: \(storedServerFingerprints)")
      ...
      Diagnostics.debug("Stored account keys: \(storedAccountKeys)")
      ...
      Diagnostics.debug("Updated accounts list: \(updatedAccountsList)")
      ...
      Diagnostics.debug("Deleted accounts: \(accountsToRemove)")
      ...
      Diagnostics.debug("Deleted account profiles: \(accountProfilesToRemove)")
      ...
      for accountID in mfaTokensToRemove {
        ...
        case let .failure(error):
          throw
            error
            .logged(
              info: .message("Failed to delete account mfa token")
            )
        }
      }
      Diagnostics.debug("Deleted account mfa tokens: \(mfaTokensToRemove)")
      ...
      Diagnostics.debug("Deleted server fingerprints: \(serverFingerprintsToRemove)")
      ...
      Diagnostics.debug("Deleted account private keys: \(keysToRemove)")

      if updatedAccountsList.isEmpty {
        Diagnostics.debug("Deleting stored passphrases")
      ...
      Diagnostics.debug("Stored databases: \(storedDatabases)")
