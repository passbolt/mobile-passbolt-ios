...
// Actor used to synchronize access to
// state of the current user session.
// Each operation requesting currently used
// account or session tokens has to be performed
// with this actor.
@globalActor
public final actor AccountSessionActor {

  public static var shared: AccountSessionActor = .init()

  public static func execute<Success>(
    _ operation: @AccountSessionActor @escaping () async throws -> Success
  ) async throws -> Success {
    try await Task { @AccountSessionActor in
      try await operation()
    }
    .value
  }
  ...
  public static func executeDetached(
    ...
    Task.detached { @AccountSessionActor in
      try await operation()
    }
  }
